...
function validRomans(arr) {
  const res = [];
  const regExp =
    /^M{0,4}(D?C{0,4}|C{1,2}D|C{1,2}M)(L?X{0,4}|X{1,2}L|X{1,2}C)(IX|IV|V?I{0,3})$/;

  arr.forEach((element) => {
    if (regExp.test(element) && element != "") res.push(element);
  });
...
// <-- Best Solution -->
function validRomansBest(arr) {
  const regex = /^M{0,4}(C[DM]|D?C{0,3})(X[CL]|L?X{0,3})(I[XV]|V?I{0,3})$/;
