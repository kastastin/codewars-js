...
/*
  You are given string elements and an int n. Your task is to return a string that is a palindrome using elements from the string elements with the length n.
...
function palindromization(element, n) {
  ...
  const firstHalf = element
    .repeat(Math.ceil(halfLength / element.length))
    .slice(0, halfLength);

  return (
    firstHalf + [...firstHalf.slice(0, Math.floor(n / 2))].reverse().join``
  );
}

// <-- Best Solution -->
function palindromizationBest(s, n) {
