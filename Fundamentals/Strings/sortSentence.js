...
function sort(sentence) {
  ...
  sentence.split(/\W+/).forEach((word) => {
    if (word) {
      word[0] === word[0].toUpperCase() ? capitalWords.push(word) : lowerCaseWords.push(word);
    }
  });

  const lower = lowerCaseWords.sort((a, b) => (a < b ? -1 : 1)).join(" ");
  const capital = capitalWords.sort((a, b) => (a > b ? -1 : 1)).join(" ");

  return `${lower} ${capital}`.trim();
}
