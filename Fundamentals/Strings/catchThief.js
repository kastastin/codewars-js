...
function catchThief(queue) {
  ...
  for (let i = 0; i < queue.length; i++) {
    if (/\d/.test(queue[i])) {
      const first = Math.max(0, i - +queue[i]);
      const second = Math.min(i + +queue[i], queue.length);
...
// <-- Best Solution -->
function catchThiefBest([...q], c = 0) {
  return q.map((v, i) => v == "X" && q.some((v, j) => Math.abs(i - j) <= v - 0) && ++c) && c;
}
