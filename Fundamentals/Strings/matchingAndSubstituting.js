...
// <-- Solution -->
const PHONE = /^Phone: \+1-\d{3}-\d{3}-\d{4}$/m;
const VERSION = /^Version: (\d+\.\d+)$/m;
const DEFAULTS = "Author: g964 Phone: +1-503-555-0090 Date: 2019-01-01";

function change(s, prog, version) {
  if (!VERSION.test(s) || !PHONE.test(s)) {
    return "ERROR: VERSION or PHONE";
  }

  const ver = s.match(VERSION)[1];
  return `Program: ${prog} ${DEFAULTS} Version: ${ver === "2.0" ? ver : version}`;
}
