...
function stat(strg) {
  ...
  const getStr = (n) =>
    addZero(Math.floor(n / 3600)) +
    "|" +
    addZero(Math.floor((n % 3600) / 60)) +
    "|" +
    addZero(n % 60);
  ...
  var median =
    len % 2 === 1
      ? arr[(len - 1) / 2]
      : Math.floor((arr[len / 2] * 1 + arr[len / 2 - 1] * 1) / 2);
...
function stat(strg) {
  let times = strg
      .split(", ")
      .map((e) => e.split("|").reduce((p, c) => p * 60 + Number(c)))
      .sort((a, b) => a - b),
    length = times.length,
  ...
  return `Range: ${final(range)} Average: ${final(average)} Median: ${final(
    median,
  )}`;
}
