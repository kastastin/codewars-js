...
function ipToNum(ip) {
  return parseInt(
    ip.split(".").reduce(function (prev, cur) {
      let lBin = parseInt(cur).toString(2);

      return prev + Array(9 - lBin.length).join("0") + lBin;
    }, ""),
    2,
  );
}
function numToIp(num) {
  ...
  for (let i = 0; i < bin.length; i += 8) {
    out.push(parseInt(bin.substring(i, i + 8), 2).toString(10));
  }
