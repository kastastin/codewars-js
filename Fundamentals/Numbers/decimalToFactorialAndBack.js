...
// <-- Solution -->
function dec2FactString(dec) {
  let result = "";
  let divider = 1;

  while (dec > 0) {
    const remainder = dec % divider;
    result = (remainder > 9 ? remainder.toString(16).toUpperCase() : remainder) + result;
    dec = Math.floor(dec / divider++);
  }
...
function factString2Dec(str) {
  return str
    .split("")
    .map((char) => (isFinite(char) ? +char : parseInt(char, 16)))
    .reduce((accum, item, index, arr) => (arr.length - index) * accum + item);
}
