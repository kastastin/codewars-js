...
function prefixSumsToSuffixSums(prefixSums) {
  const res = [];
  const lastNum = prefixSums.pop();
  
  res.push(lastNum);

  prefixSums.forEach((num) => {
    res.push(lastNum - num);
  });
...
// <-- Best Solution -->
function prefixSumsToSuffixSumsBest(prefixSums) {
  const suffixSums = prefixSums.slice();
  const sum = suffixSums.pop();

  return [sum].concat(suffixSums.map((value) => sum - value));
}
