...
function findBall(scales) {
  let panel = [0, 1, 2, 3, 4, 5, 6, 7];

  while (panel.length != 1) {
    const div = Math.ceil(panel.length / 3);
    const w = scales.getWeight(panel.slice(0, div), panel.slice(-div));

    panel = w == 0 ? panel.slice(div, -div) : w == -1 ? panel.slice(0, div) : panel.slice(-div);
  }
