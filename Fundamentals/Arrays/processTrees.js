...
// <-- Solution -->
function getChildren(processes, pid) {
  return processes.filter((x) => x.ppid == pid).map((x) => new Process(x.pid, getChildren(processes, x.pid)));
}

function makeProcessTree(processes) {
  return getChildren(processes, -1)[0];
}
