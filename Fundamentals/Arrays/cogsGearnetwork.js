...
function cogsebi(gears, connections, driverId, driverRpm) {
  const connx = connections.reduce(
    (acc, [g1, g2]) => (acc[g1].push(g2), acc[g2].push(g1), acc),
    gears.map(() => []),
  );

  const result = gears.map(() => 0);
  result[driverId] = driverRpm;

  const queue = [driverId];
  while (queue.length) {
    const id0 = queue.shift();

    const term = -result[id0] * gears[id0];

    if (!term) {
      continue;
    }

    connx[id0].forEach((id1) => (result[id1] ||= queue.push(id1) && term / gears[id1]));
  }
