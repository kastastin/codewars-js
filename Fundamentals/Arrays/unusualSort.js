...
// <-- Best Solution -->
function unusualSortBest(arrayOfChars) {
  const a = arrayOfChars.filter((v) => /[a-z]/.test(v)).sort();

  const x = arrayOfChars
    .filter((v) => /[A-Z]/.test(v))
    .sort((a, b) => a.localeCompare(b));

  const n = arrayOfChars
    .filter((v) => /[0-9]/.test(v))
    .sort((a, b) => a - b || typeof a === "string");

  return x.concat(a).concat(n);
}
