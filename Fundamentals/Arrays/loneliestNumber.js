...
function loneliest(number) {
  ...
  let lonelinessDigit = null;

  for (let i = 0, loneliness = Infinity; i < numberStr.length; i++) {
    let digit = +numberStr[i];
    let left = numberStr.substring(i - digit, i);
    let right = numberStr.substring(i + 1, i + 1 + digit);

    let sum = (left + right).split("").reduce((acc, cur) => acc + +cur, 0);

    if (sum < loneliness || (sum === loneliness && lonelinessDigit !== "1")) {
      loneliness = sum;
      lonelinessDigit = digit + "";
    }
  }

  return lonelinessDigit === "1";
}
