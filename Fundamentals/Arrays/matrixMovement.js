...
function move(matrix) {
  const next = {
    up: (m) => m.slice(1).concat([m[0]]),
    down: (m) => [m[m.length - 1]].concat(m.slice(0, -1)),
    left: (m) => m.map((row) => row.slice(1).concat(row[0])),
    right: (m) => m.map((row) => [row[row.length - 1]].concat(row.slice(0, -1))),
  };

  return (command) => (command == "stop" ? matrix : move(next[command](matrix)));
}
