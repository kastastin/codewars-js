...
function bucketize(arr) {
  const itemToEntryCountMap = arr.reduce((acc, item) => {
  ...
  for (let number in itemToEntryCountMap) {
    const entryCount = itemToEntryCountMap[number];
    const resultBucket = result[entryCount] || [];

    resultBucket.push(Number(number));
    result[entryCount] = resultBucket;
  }
...
// <-- Best Solution -->
function bucketizeBest(arr) {
  const res = Array(arr.length + 1)
    .fill()
    .map((_) => []);
  new Set(arr.sort((a, b) => a - b)).forEach((v) => {
    res[arr.filter((int) => int === v).length].push(v);
  });

  return res.map((v) => (v.length ? v : null));
}
