...
function allocateRooms(customers) {
  const rooms = new Map(); // k = room - v = depart

  const indexedOriginalSorted = customers.map((c, i) => ({ c, i }));

  indexedOriginalSorted.sort((a, b) => a.c[0] - b.c[0]);

  const returnArr = new Array(customers.length); //set room in original order

  indexedOriginalSorted.forEach(({ c, i }) => {
    const freeRooms = [...rooms.entries()].find(([room, end]) => end < c[0]);

    if (freeRooms) {
      rooms.set(freeRooms[0], c[1]);
      returnArr[i] = freeRooms[0];
    } else {
      returnArr[i] = rooms.size + 1;
      rooms.set(rooms.size + 1, c[1]);
    }
  });
