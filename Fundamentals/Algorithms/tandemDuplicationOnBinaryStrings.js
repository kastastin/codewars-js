...
/*
  ...
  So a valid output is: [ [0, 2], [1, 4] ]
*/

// <-- Solution -->
function binaryDuplication(bits) {
  if (bits[0] !== "0" || bits.at(-1) !== "1") {
    return null;
  } else {
    let r = (bits.match(/10/g) ?? []).map(() => [0, 2]);
    let j = 0;

    [].forEach.call(bits, (bit, i) => (bit === "01"[j] ? (j ^= 1) : r.push([i - 1, i])));
