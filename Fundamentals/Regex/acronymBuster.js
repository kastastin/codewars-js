...
function acronymBuster(string) {
  ...
  const abr = string.match(/\b[A-Z]{3,}\b/g) || [];

  return abr.every((e) => obj[e])
    ? string.replace(/\b[A-Z]{3,}\b/g, (x) => obj[x]).replace(/\. [a-z]|^[a-z]/g, (x) => x.toUpperCase())
    : abr.find((e) => !obj[e]) + " is an acronym. I do not like acronyms. Please remove them from your email.";
}
